...
\end{aligned}
\]
where~$[\hat{\mathbf{w}}_{u},\hat{\mathbf{w}}_{v}]$ are the parameters. This vector maximizes the likelihood function~\eqref{eq:sb:8}, and~$\hat{\mathbf{V}}_u$ is~a~part of the covariance matrix~$\hat{\mathbf{V}}=\mathbf{I}^{-1}\bigr(\mathfrak{D}_m, [\hat{\mathbf{w}}_{u},\hat{\mathbf{w}}_{v}]\bigr)$, corresponding to~$\hat{\mathbf{w}}_{u}$.
...
\begin{figure}[!hbp]
\centering{\includegraphics[width=0.85\textwidth]{fig2}}%{graphs.png}}
